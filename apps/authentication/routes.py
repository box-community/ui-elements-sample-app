...
from apps.authentication.bootstrap import boot_strap
...
@blueprint.route("/bootstrap", methods=["GET", "POST"])
def bootstrap():
    boot_strap()
    return redirect(url_for("authentication_blueprint.login"))
...
def login():
    ...
    return redirect(url_for("booking_blueprint.page_home"))
    ...
    if "register" in request.form:
        ...
        if user:
            return render_template(
                "accounts/register.html", msg="User name already registered", success=False, form=create_account_form
            )
